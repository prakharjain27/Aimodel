...
import { Sparkles, ArrowLeft, Upload, Loader2, HelpCircle, Coins } from 'lucide-react'
...
export default function EditCharacterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const checkUserAndFetchChar = async () => {
      ...
      if (isDemoMode) {
        ...
        setName('Demo Character')
        setAge(24)
        setSkinTone('Medium/Tan')
        setBodyType('Curvy')
        setHairStyle('Short dark bob')
        setEyeColor('Brown')
        setFaceFeatures('Sharp jawline')
        setTattoos('None')
        setBirthmarks('Beauty mark on left cheek')
        setSignaturePose('Looking over shoulder')
        setStyleVibe('Streetwear chic')
      ...
      setFaceFeatures(char.face_features || '')
      setTattoos(char.tattoos || '')
      setBirthmarks(char.birthmarks || '')
      setSignaturePose(char.signature_pose || '')
      setStyleVibe(char.style_vibe)
  ...
  if (initialLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center text-violet-500">
  ...
  return (
    <div className="flex-1 min-h-screen bg-slate-950 text-slate-100 flex flex-col relative overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto w-full px-6 py-10 space-y-8 relative z-10 flex-1">
        ...
        {/* Title */}
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Edit Character</h1>
          <p className="text-sm text-slate-400">Update the physical features and characteristics of your virtual model.</p>
        </div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <span>Save Character Changes</span>
                </>
              )}
            </button>
          </div>
